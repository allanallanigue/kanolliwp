...
class Woocommerce_Catalog_Mode {
	...
	public function set_up_template_add_to_cart(){
		if($this->isCategoriesExludedFromCatalog()){//excluded categories bring back add to cart
	...
	public function set_up_template_price(){
			...
			if($this->isCategoriesExludedFromCatalog()){
	...
	public function isCategoriesExludedFromCatalog() {
		
		global $product;
		$terms = get_the_terms( $product->id, 'product_cat' );
		if(!in_array("all",$this->woo_categories)){
			if($terms){
				$cat_ids=array_keys($terms);
				foreach ($cat_ids as $c_id){
				if(!in_array($c_id,$this->woo_categories)){
						return true;
					}
				}
